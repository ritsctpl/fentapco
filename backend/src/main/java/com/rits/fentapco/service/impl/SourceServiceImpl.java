...
public class SourceServiceImpl implements SourceService {
     ...
     * public boolean testConnection(Source source) {
     * try {
     * // Build the URI for the OPC UA source
     * String endpointUrl = source.getEndpointUrl();
     * 
     * // Create an OPC UA client
     * OpcUaClient client = OpcUaClient.create(endpointUrl);
     ...
     * System.out.println("Successfully connected to OPC UA server: " +
     * endpointUrl);
     * return true;
     * } catch (Exception e) {
     * // Log the exception and return false
     * System.err.println("Failed to connect to OPC UA server: " +
     ...
     * // List<SourceNode> nodes = new ArrayList<>();
     * // if ("opcua".equalsIgnoreCase(source.getType())) {
     * // return discoverOpcUaNodes(source);
     * // } else {
     * // throw new UnsupportedOperationException(
     * // "Node discovery is not supported for protocol: " + source.getType());
     * // }
     * // }
     * 
     * 
     ...
     * client.disconnect().get();
     * 
     * System.out.println("Node discovery successful for OPC UA server: " +
     * endpointUrl);
     * } catch (Exception e) {
     * System.err.println("Failed to discover nodes for OPC UA server: " +
     * source.getEndpointUrl());
     * e.printStackTrace();
    ...
    /*
     * @Override
     * public List<SourceNode> discoverCompleteNodeHierarchy(Source source) {
     * List<SourceNode> allNodes = new ArrayList<>();
     * try {
     * // Build the endpoint URL for the OPC UA source
     * String endpointUrl = source.getEndpointUrl();
     * 
     ...
     * // Start the recursive discovery from the root node
     * NodeId rootNodeId = Identifiers.RootFolder;
     * browseNodeHierarchy(client, rootNodeId, allNodes);
     * 
     * // Disconnect the client
     * client.disconnect().get();
     * 
     * System.out.println("Complete node hierarchy discovery successful.");
     * } catch (Exception e) {
     * System.err.println("Failed to discover complete node hierarchy for source: "
     * + source.getEndpointUrl());
     * e.printStackTrace();
     * throw new RuntimeException("Failed to discover complete node hierarchy: " +
     * e.getMessage());
     * }
     * return allNodes;
     * }
     * 
     * private void browseNodeHierarchy(OpcUaClient client, NodeId parentNodeId,
     ...
     * if (nodeId != null) {
     * SourceNode sourceNode = new SourceNode();
     * sourceNode.setNodeId(nodeId.toParseableString());
     * sourceNode.setDisplayName(reference.getDisplayName().getText());
     * nodeList.add(sourceNode);
     * 
     * // Recursively browse child nodes
     * browseNodeHierarchy(client, nodeId, nodeList);
     * }
     * }
     * }
     */
